...
| DataPort                | Port used By DbEngine              | Specify only if different from the default |
...
The following values are used to <u>lookup</u> the existing database proxy. No database proxy will be created.

| Name                       | Description                | Instructions              |
| -------------------------- | -------------------------- | ------------------------- |
| DatabaseProxyArn           | ARN of the Database Proxy  | Required if using a proxy |
| DatabaseProxyEndpoint      | Proxy Endpoint             | Required if using a proxy |
| DatabaseProxyName          | Name of the proxy          | Required if using a proxy |
| DatabaseProxySecurityGroup | Database Security Group ID | Required if using a proxy |
